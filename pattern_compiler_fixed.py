#!/usr/bin/env python3
"""
BandLang Pattern Compiler FIXED - Sistema Musical + Patterns Rítmicos Precisos
Corrige as pausas estranhas no sistema de patterns
"""

import sys
import re

class PatternCompilerFixed:
    def __init__(self):
        ...
        self.duration_map = {
            ...
            "quarter.": 720, "half.": 1440, "eighth.": 360, "q.": 720, "h.": 1440, "e.": 360,
            # Frações úteis
    ...
    def compile_bandlang(self, content):
        ...
        # Header
        gbasm.append("; Generated by BandLang Pattern Compiler FIXED")
        gbasm.append("; Musical notation + Rhythmic patterns (no weird pauses)")
        gbasm.append("")
    ...
    def generate_pattern_gbasm(self, pattern_name):
        """Gera GBASM para um pattern - VERSÃO OTIMIZADA SEM PAUSAS DESNECESSÁRIAS"""
        ...
        # Estratégia otimizada: acumula waits de steps vazios
        i = 0
        while i < pattern['steps']:
            # Verifica se há eventos no step atual
            has_events = False
            for drum_type, steps in pattern['tracks'].items():
                if i < len(steps) and steps[i] > 0:
                    has_events = True
                    break
            
            if has_events:
                # Gera eventos para este step
                gbasm.append(f"; Step {i + 1}")
                ...
                # Conta quantos steps vazios consecutivos há
                empty_steps = 0
                j = i
                while j < pattern['steps']:
                    step_has_events = False
                    for drum_type, steps in pattern['tracks'].items():
                        if j < len(steps) and steps[j] > 0:
                            step_has_events = True
                            break
                ...
                # Gera um WAIT acumulado para todos os steps vazios
                if empty_steps > 0:
                    total_wait = empty_steps * step_duration
                    gbasm.append(f"; Skip {empty_steps} empty steps")
                    gbasm.append(f"WAIT {total_wait}")
                    i += empty_steps
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python pattern_compiler_fixed.py <input.band> <output.gbasm>")
        sys.exit(1)
    ...
    try:
        ...
        compiler = PatternCompilerFixed()
        gbasm = compiler.compile_bandlang(content)
        
        with open(output_file, 'w') as f:
            f.write(gbasm)
        
        print(f"✓ FIXED Pattern compilation: {input_file} -> {output_file}")
